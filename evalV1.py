...
def evalulate_models(data_loader, model, config, client, plot_suffix="_Test", mode='train', z_train=None, y_train=None, nData=None):
    ...
    #     z_l_, clabels_l_ = append_tensors_to_lists([z_l_, clabels_l_],
    #                                              [x, label.int()])
    ...
    z_ = concatenate_lists([z_l_])
    clabels_ = concatenate_lists([clabels_l_])
    ...
    if mode == 'test':
        ...
        print(z_train[1].shape,z_.shape)
        print(f" Evaluate original dataset")
        linear_model_eval(config, z_train[1], y_train[1],"Client-" + str(client) + suffix + "-original-", z_test=z_, y_test=clabels_, description=description, nData=nData)
        
        print(z_train[0].shape,z.shape)
        print(f" Evaluate embeddings dataset")
        linear_model_eval(config, z_train[0], y_train[0],"Client-" + str(client) + suffix + "-contrastive-", z_test=z, y_test=clabels, description=description, nData=nData)# linear_model_eval(config, z, clabels,"Client-" + str(client) + "-contrastive-", z_test=z_train, y_test=y_train, description=description)
        

    else:
        # Return z_train = z, and y_train = clabels
        return [z,z_], [clabels,clabels_]
