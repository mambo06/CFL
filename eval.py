...
def eval(data_loader, config, client, nData):
    ...
    with th.no_grad():          
        if  config['original_dataset']:  
            ...
            if nData != None:
                for item in nData:
                    ...
                    test_result = evalulate_original(data_loader, config, client, plot_suffix="test", mode="test", z_train=None, y_train=None, nData=item)
            else :
                test_result = evalulate_original(data_loader, config, client, plot_suffix="test", mode="test", z_train=None, y_train=None)

            results_list[0] = copy.deepcopy(test_result)
            test_result = None
        ...
        if nData != None:
            for item in nData:
                # Train and evaluate logistig regression using the joint embeddings of training and test set
                test_result = evalulate_models(data_loader, model, config, client, plot_suffix="test", mode="test", z_train=None, y_train=None, nData=item)
        else:
            test_result = evalulate_models(data_loader, model, config, client, plot_suffix="test", mode="test", z_train=None, y_train=None)

        results_list[1] = copy.deepcopy(test_result)
        test_result = None
...
def evalulate_models(data_loader, model, config, client, plot_suffix="_Test", mode='train', z_train=None, y_train=None, nData=None):
    ...
    break_line = lambda sym: f"{10 * sym}\n"
    ...
    if mode == 'test':
        ...
        test_result = model_eval(config, z_train, y_train,"Client-" + str(client) + suffix + "-contrastive-", z_test=z, y_test=clabels, description=description, nData=nData)# model_eval(config, z, clabels,"Client-" + str(client) + "-contrastive-", z_test=z_train, y_test=y_train, description=description)
        return test_result
    else:
...
def evalulate_original(data_loader, config, client, plot_suffix="_Test", mode='train', z_train=None, y_train=None, nData=None):
    ...
    if mode == 'test':
        ...
        test_result = model_eval(config, z_train, y_train,"Client-" + str(client) + suffix + "-original-", z_test=z, y_test=clabels, description=description, nData=nData)
        return test_result
    else:
...
def main(config,client=1, nData=None):
    ...
    # Start evaluation
    results = eval(ds_loader, config, client, nData)
    return results
